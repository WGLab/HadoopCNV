...
### 4) After all above is done, just run by `./run.sh`

### 5) To run many samples in a sequencial pipeline, please see our example `run_all.pl`. The idea is to use `sed` to change parameters each time.
