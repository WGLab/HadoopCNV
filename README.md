...
2) In general, there is no need to change any other parameters in the config.txt file any more. 
...
For example, if your file is called 'NA12878.chrom*.ILLUMINA.bwa.CEU.high_coverage.20100311.bam', you should change 'hg19.bam.extra' to 'NA12878.chrom.ILLUMINA.bwa.CEU.high_coverage.20100311.bam.extra' in HDFS, and change 'BAM_FILE' parameter in your config.txt to 'NA12878.chrom*.ILLUMINA.bwa.CEU.high_coverage.20100311.bam.*'. Then this additional file is automatically included.
...
First put all fasta files of your specified genome into a folder *./fasta*

Then run this script by `python preprocessBAM.py <prefix> ./fasta`. A SAM file will be generated as '<prefix>.extra.sam'.
Then use [Samtools](http://www.htslib.org/) to transform it to BAM file by: `samtools -t fasta/chrall.fa.fai <prefix>.extra.sam > NA12878.chrom.ILLUMINA.bwa.CEU.high_coverage.20100311.bam.extra`

<prefix> can be any name you give to the file. 'fasta/chrall.fa.fai' is the fasta index associated with all chromosomes. 
